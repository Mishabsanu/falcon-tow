    ...
    <tr key={tow._id || tow.id}>
      ...
      <td>
        <div className={styles.dateCell}>
          ...
          <span>{new Date(tow.date).toLocaleDateString()}</span>
        </div>
      </td>
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">REF #{tow.id}</p>
      ...
      <div className="grid grid-cols-2 gap-4 py-4 border-y border-emerald-50">
        <div>
          ...
          <p className="text-xs font-bold text-emerald-950">{tow.customer}</p>
        </div>
        <div>
          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1">Amount</p>
          <p className="text-xs font-black text-emerald-600">QAR {tow.amount}</p>
        </div>
      </div>
