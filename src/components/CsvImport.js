...
export default function CsvImport({ moduleKey, onComplete }) {
  ...
  const downloadTemplate = async () => {
    ...
    // Fetch current valid values for dropdowns to provide as "Hints"
    let hintRows = [];
    const hintData = {};
      ...
      for (const field of dropdownFields) {
        if (field.module) {
          try {
            ...
            hintData[field.name] = (result.data || []).map(d => {
              if (field.module === 'vehicles') return `${d.name} - ${d.plate}`;
              return d.name;
            });
          } catch (e) {
            console.error(`Failed to fetch hints for ${field.name}:`, e);
          }
        } else if (field.options) {
          hintData[field.name] = field.options;
        }
      }

      // Build a few hint rows
      const hintArrays = Object.values(hintData);
      const maxHints = hintArrays.length > 0 ? Math.max(...hintArrays.map(v => v.length), 0) : 0;
      for (let i = 0; i < Math.min(maxHints, 5); i++) {
        const row = visibleFields.map(f => {
          const hints = hintData[f.name] || [];
          return hints[i] ? `"${hints[i].replace(/"/g, '""')}"` : '';
        }).join(',');
        hintRows.push(row);
      }
    } catch (error) {
      console.error('Core hint engine failure:', error);
    }
              ...
              >
                ...
                <div className="bg-emerald-50/50 rounded-2xl p-6 border border-emerald-100 flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                  <div className="flex-1 text-center sm:text-left">
                    <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-emerald-900 mb-1">Architecture Reference</h4>
                    <p className="text-[10px] text-emerald-700/60 font-bold">Download the template and valid dropdown values to ensure 100% data accuracy.</p>
                  </div>
                ...
                <div className="space-y-14 flex-1 overflow-y-auto pr-2">
                  ...
                  <div className="space-y-5">
                    ...
                    >
                      ...
                      <span>Download CSV Structure</span>
                    </button>
                  </div>
